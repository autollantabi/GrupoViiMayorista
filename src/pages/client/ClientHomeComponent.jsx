...
import { empresas, getEmpresaLogo, lineasNegocio } from "../../mock/products";
import Button from "../../components/ui/Button";
import { api_products_getInfoProductos } from "../../api/products/apiProducts";
import { api_cart_createCarrito } from "../../api/cart/apiCart";
...
import SeleccionClienteModal from "../../components/catalog/SeleccionClienteModal";
...
const ClientHomeComponent = () => {
  ...
  const { registerSellerCart, loadSellerCartFromDetails, clearSellerData } = useCart();
  ...
  useEffect(() => {
    ...
    const fetchAccessSections = async () => {
      try {
        ...
        if (!user || !user.EMAIL || isSeller) return;
  ...
  const userAccessLinea = user?.LINEAS || [];
  ...
  // Abre el modal de selección de cliente
  const handleVerCatalogo = () => {
    setShowClientModal(true);
  };

  // Vendedor seleccionó un cliente concreto → crea carritos para TODAS las empresas y navega
  const handleClientSelect = async (client) => {
    setShowClientModal(false);
    setIsCreatingCart(true);

    try {
      // Limpiar todos los datos del cliente anterior antes de crear el nuevo carrito
      clearSellerData();

      const linea = user?.LINEAS?.[0];
      if (!linea) {
        setIsCreatingCart(false);
        return;
      }

      // Crear carritos para TODAS las empresas autorizadas del vendedor
      const empresas = user?.EMPRESAS || [];
      const clientAccount = client.ACCOUNT_USER;
      let firstEnterprise = "";

      const cartPromises = empresas.map(async (enterprise) => {
        try {
          const cartResult = await api_cart_createCarrito(clientAccount, enterprise);
          const header = cartResult?.data?.header;

          if (header?.ID_SHOPPING_CART_HEADER && header?.ENTERPRISE && header?.ACCOUNT_USER) {
            registerSellerCart(
            ...
            // Cargar productos existentes del carrito (details de la respuesta)
            const details = cartResult?.data?.details;
            if (Array.isArray(details) && details.length > 0) {
              await loadSellerCartFromDetails(details, header.ENTERPRISE);
            }

            return header.ENTERPRISE;
          }
        } catch (error) {
          console.error(`Error al crear carrito para ${enterprise}:`, error);
        }
        return null;
      });

      const results = await Promise.all(cartPromises);
      // Usar la primera empresa exitosa para la navegación
      firstEnterprise = results.find(Boolean) || client.ENTERPRISE || empresas[0] || "";

      navigate(`/vendedor/${linea}`);
    } catch (error) {
  ...
  // Vendedor saltó la selección de cliente
  // Vendedor saltó la selección de cliente
  const handleSkipClientSelect = () => {
    setShowClientModal(false);
    setIsCreatingCart(true);

    try {
      // Limpiar cualquier carrito de cliente previo al entrar al catálogo general
      clearSellerData();
      ...
      navigate(`/vendedor/${linea}?dma_empresa=${empresa.toUpperCase()}`);
    } catch (error) {
      console.error("Error al saltar selección de cliente:", error);
      setIsCreatingCart(false);
    }
  };
  ...
  if (isLoading || isCreatingCart) {
    return (
      <LoadingContainer>
        ...
        <LoadingText>
          {isCreatingCart ? "Preparando catálogo del vendedor..." : "Cargando empresas..."}
        </LoadingText>
      </LoadingContainer>
    );
  }
  ...
  return (
    <>
      {/* Modal de selección de cliente para vendedores */}
      <SeleccionClienteModal
        isOpen={showClientModal}
        onClose={() => setShowClientModal(false)}
        onSelectClient={handleClientSelect}
        onSkip={handleSkipClientSelect}
      />
      <SEO
      ...
      {!isSeller ? (
        < CompaniesSection id="empresas-grid">
          <SectionTitle>Explora Nuestros Catalogos</SectionTitle>
          <SectionSubtitle>
          ...
          <CompaniesGrid>
            {empresas.map((empresa) => {
              ...
              return (
                <CompanyCard
                  key={empresa.id}
                  onClick={() => handleCardClick(empresa, false)}
                  $hasAccess={hasAccess}
                >
        ...
        <FooterContainer>
          ...
          <FooterSection>
            ...
            {!isSeller ? (
              ...
              >
                <RenderIcon name="FaStore" size={14} />
                {user?.LINEAS?.[0] || "Ninguna linea disponible"}
              </FooterLink>
            )}
          </FooterSection>
