...
const CompanySelectionHeader = styled.div`
  text-align: center;
  max-width: 800px;
  margin: 0 auto;
`;

const CompanySelectionTitle = styled.h2`
  font-size: 2rem;
  font-weight: 800;
  color: ${({ theme }) => theme.colors.text};
...
const CompanySelectionDescription = styled.p`
  font-size: 1.1rem;
  color: ${({ theme }) => theme.colors.textSecondary};
  line-height: 1.6;
`;
...
const CompanyCardStyled = styled.div`
  ...
  &:hover {
    ...
    border-color: ${({ theme }) => theme.colors.primary};
    box-shadow: 0 12px 32px ${({ theme }) => `${theme.colors.primary}20`};
...
const CompanyLogoWrapper = styled.div`
  ...
  border-bottom: 1px solid ${({ theme }) =>
    theme.mode === "dark" ? `${theme.colors.border}20` : `${theme.colors.border}10`};
  ...
  ${CompanyCardStyled}:hover & img {
    transform: scale(1.1);
  }
`;

const CompanyCardBody = styled.div`
  padding: 1.5rem;
  flex: 1;
  display: flex;
  flex-direction: column;
...
const CompanyNameStyled = styled.h3`
  font-size: 1.25rem;
  font-weight: 700;
  color: ${({ theme }) => theme.colors.text};
  margin: 0;
...
const CompanyDescriptionStyled = styled.p`
  font-size: 0.9rem;
  color: ${({ theme }) => theme.colors.textSecondary};
  margin: 0;
  line-height: 1.4;
...
const CatalogVendedor = () => {
  const { lineaID: empresaName } = useParams();
  const navigate = useNavigate();
  const location = useLocation();
  const [searchParams, setSearchParams] = useSearchParams();
  const { user, isSeller } = useAuth();
  const { selectedClientName } = useCart();
  const {
  ...
  // Filtrar empresas que el usuario vendedor tiene disponibles
  const userAuthorizedCompanies = user?.EMPRESAS || [];
  const authorizedCompaniesData = empresas.filter(emp =>
    userAuthorizedCompanies.includes(emp.nombre.toUpperCase())
  );
  ...
  useEffect(() => {
    ...
    if (
      ...
      currentUrl.startsWith("/vendedor")
    ) {
  ...
  // Cargar productos de la empresa o del vendedor según el rol
  // Para vendedores: cargar todos los productos (sin paginación)
  const lastSellerRequestRef = React.useRef("");

  useEffect(() => {
    if (isSeller) {
      const urlSort = searchParams.get("sort") || "default";

      // Determinar ordenamiento API
      let ordenamiento = "DEFAULT";
      if (urlSort === "clasificacion_indice") ordenamiento = "CLASIFICACION_INDICE";
      else if (urlSort === "recurrencia") ordenamiento = "RECURRENCIA";

      // Crear una clave para evitar llamadas duplicadas
      const requestKey = `${ordenamiento}`;
      if (requestKey === lastSellerRequestRef.current) return;
      lastSellerRequestRef.current = requestKey;

      loadProductsForSeller({ ordenamiento });
    } else if (empresaName && !catalogByEmpresa[empresaName]) {
      loadProductsForEmpresa(empresaName);
    }
  }, [isSeller, empresaName, searchParams.toString()]);
  ...
  const catalogProducts = useMemo(() => {
    // Vendedor: usar siempre el catálogo propio del endpoint de vendedor
    if (isSeller) return sellerCatalog.length > 0 ? sellerCatalog : null;
    if (empresaName) return catalogByEmpresa[empresaName] || null;
    return null;
  }, [isSeller, sellerCatalog, empresaName, catalogByEmpresa]);
  ...
  const handleProductSelect = (product) => {
    ...
    const currentUrl = `/vendedor/${empresaName || ""}${location.search}`;
  ...
  // Verificar si el usuario tiene acceso (línea para vendedor, empresa para cliente)
  if (empresaName && user) {
    const empresaNameUpper = empresaName.toUpperCase();
    let hasAccess = false;
    let accessType = "empresa";

    if (isSeller) {
      const userLines = user?.LINEAS || [];
      hasAccess = userLines.includes(empresaNameUpper);
      accessType = "línea de negocio";
    } else {
      const userCompanies = user?.EMPRESAS || [];
      hasAccess = userCompanies.includes(empresaNameUpper);
      accessType = "empresa";
    }
                ...
                <UnauthorizedContent>
                  <UnauthorizedLeft>
                    ...
                    <TitleWithIcon>
                      ...
                      <UnauthorizedTitle>
                        Solicitar Acceso a {accessType === "empresa" ? empresaName : `Línea ${empresaName}`}
                      </UnauthorizedTitle>
                    </TitleWithIcon>
                    <UnauthorizedMessage>
                      No tienes permisos para acceder a la {accessType}{" "}
        ...
        <MainContent>
          <WelcomeScreen>
            <WelcomeHeader>
              ...
              <WelcomeDescription>
                {isSeller
                  ? "Selecciona una línea de negocio para explorar los productos disponibles."
                  : empresaName
        ...
        <MainContentProducts>
          <ContentWithFilters>
            ...
            <ProductGridView
              ...
              loading={loading || loadingSellerCatalog}
              empresaName={empresaName}
              onReloadProducts={isSeller
                ? (_, ordenamiento) => loadProductsForSeller({ ordenamiento })
                : reloadProductsForEmpresa
              }
              serverTotal={null}
...
export default CatalogVendedor;
